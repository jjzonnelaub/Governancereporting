...
function writeGovernanceDataToSheet(processedData, programIncrement, valueStream, sheetName, clearExisting = false, isFirstValueStream = false, targetSpreadsheet = null) {
  ...
  const headers = [
    ...
    'Infosec Subsection', 'Initiative Title', 'Initiative Description', 'Investment (Story Points)', 'Story Point Completion', 
    'Total Child Issues', 'Closed Child Issues', '% Complete', 'Business Value', 'Actual Value', 'RAG', 'RAG Note', 
    'Depends on Valuestream', 'Depends on Team', 'Fix Versions', 'Start Iteration Name', 'End Iteration Name', 
    'Feature Points', 'Governance', 'Momentum', 'Created Date', 'Updated Date', 'Last Refresh'
  ];
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // ✅ ENRICHMENT CACHE: Cache columns AO+ BEFORE any clearing
  ...
  if (!isFirstWrite && sheet.getLastRow() > 4) {
    ...
    if (lastCol >= ENRICHMENT_START_COLUMN) {
      console.log(`\n📦 ENRICHMENT CACHE: Caching columns ${columnToLetter(ENRICHMENT_START_COLUMN)} to ${columnToLetter(lastCol)}...`);
      ...
      if (dataRowCount > 0) {
        ...
        console.log(`   ✓ Cached enrichment data for ${enrichmentCache.size} epics`);
      ...
      console.log(`📦 ENRICHMENT CACHE: No enrichment columns found (last column: ${lastCol})`);
    }
  }
  // ═══════════════════════════════════════════════════════════════════════════
    ...
    if (lastRow > 4) {
      ...
      if (dataRowCount > 1) {
        try {
          ...
          console.log(`🧹 FULL CLEAR: Cleared and deleted ${dataRowCount - 1} old data rows for fresh report generation`);
        } catch (e) {
          // If delete still fails, content is already cleared which is sufficient
          console.log(`🧹 FULL CLEAR: Cleared ${dataRowCount} old data rows (delete skipped: ${e.message})`);
        }
      } else {
        console.log(`🧹 FULL CLEAR: Cleared ${dataRowCount} old data row for fresh report generation`);
      }
    }
  }
  // ===== END FULL CLEAR =====
      ...
      if (excessRows > 0 && rowsToKeep.length > 0) {
        ...
        if (rowsToDelete > 0) {
          try {
            ...
            console.log(`🗑️ Deleted ${rowsToDelete} excess empty rows`);
          } catch (e) {
            // Content already cleared, deletion not critical
            console.log(`🗑️ Could not delete empty rows: ${e.message}`);
          }
        }
        ...
        if (originalRowCount > 1) {
          try {
            ...
            console.log(`🗑️ Deleted ${originalRowCount - 1} old data rows (keeping 1 empty row)`);
          } catch (e) {
            console.log(`🗑️ Could not delete rows: ${e.message}`);
          }
        }
        console.log(`🗑️ Cleared all ${originalRowCount} old data rows (no rows to keep)`);
      }
      // ===== END FIX =====
      
      console.log(`🔄 Smart clear: Removed ${rowsRemoved} rows for "${valueStream}", kept ${rowsToKeep.length} rows from other value streams`);
    }
  }
  // ===== END SMART CLEAR =====
  ...
  if (iterMatch) {
    ...
    if (refreshTimestamp) {
      sheet.getRange(3, 1).setValue(`Last Full Refresh (Fresh Data): ${refreshTimestamp.toLocaleString()}`)
        ...
        .setFontSize(9)
        .setFontColor('#666666');
      console.log(`✓ Updated timestamp in row 3: ${refreshTimestamp.toLocaleString()}`);
    } else {
      console.warn(`⚠️ No refresh timestamp found for ${reportKey}`);
    }
  }
  // ===== END TIMESTAMP CODE =====
  ...
  if (existingKeys.size > 0 && !clearExisting) {
    ...
    if (duplicateCount > 0) {
      console.log(`⚠️ Removed ${duplicateCount} duplicate keys (already in sheet)`);
  ...
  if (filteredData.length > 0) {
    ...
    const dataRows = filteredData.map(row => [
      ...
      row['Status'],
      row['Resolution'] || '',
      row['Issue Type'] === 'Epic' ? (row['Initiative Title'] || '') : '', // Epic Name column
      row['PI Objective'] || '',
      ...
      row['Infosec Subsection'] || '',
      row['Initiative Title'] || '',           // NEW: Initiative Title
      row['Initiative Description'] || '',     // NEW: Initiative Description
      row['Total Story Points'] || 0,
      row['Closed Story Points'] || 0,
      row['Total Items'] || 0,
      row['Closed Items'] || 0,
      row['% Complete (Points)'] || 0,
      row['Business Value'] || '',
      row['Actual Value'] || '',
      row['RAG'] || '',
      row['RAG Note'] || '',
    ...
    console.log(`✅ Appended ${filteredData.length} rows starting at row ${startRow} for value stream: ${valueStream}`);
  } else {
    console.log(`ℹ️ No new rows to append (all ${processedData.length} keys already exist in sheet)`);
  }
  
  // ═══════════════════════════════════════════════════════════════════════════
  // ✅ RESTORE ENRICHMENT DATA: Restore cached columns AO+ after writing new data
  // Match by Epic Key and write enrichment data back to the correct rows
  // ═══════════════════════════════════════════════════════════════════════════
  if (enrichmentCache.size > 0 && enrichmentHeaders.length > 0) {
    console.log(`\n📦 ENRICHMENT RESTORE: Restoring cached data for ${enrichmentCache.size} epics...`);
    ...
    if (dataRowCount > 0) {
      ...
      if (currentLastCol < ENRICHMENT_START_COLUMN + enrichmentHeaders.length - 1) {
        ...
        console.log(`   ✓ Created enrichment column headers: ${enrichmentHeaders.join(', ')}`);
      }
      ...
      if (enrichmentDataToWrite.length > 0) {
        ...
        console.log(`   ✓ Restored enrichment data for ${restoredCount} epics`);
        if (notFoundCount > 0) {
          console.log(`   ℹ️ ${notFoundCount} new epics without cached enrichment data`);
  ...
  console.log(`📊 Total rows in sheet now: ${sheet.getLastRow() - 4} data rows`);
}
...
function enrichWithInitiativeData() {
  return logActivity('JIRA Initiative Data Enrichment', () => {
    ...
    console.log('\n╔══════════════════════════════════════════════════════════════╗');
    console.log('║           INITIATIVE ENRICHMENT                              ║');
    console.log('╠══════════════════════════════════════════════════════════════╣');
    console.log(`║  Sheet: ${sheet.getName()}  |  ${new Date().toLocaleString()}`);
    console.log('╚══════════════════════════════════════════════════════════════╝\n');
    ...
    ss.toast('Starting Initiative enrichment...', '🔗 Fetching Data', 10);
    ...
    try {
      ...
      ss.toast(`Fetching ${initiativeKeys.size} initiatives...`, '🔗 Processing', 30);
      ...
      for (let i = 0; i < initiativeArray.length; i += BATCH_SIZE) {
        ...
        try {
          ...
          console.log(`    ✓ Got ${batchResults.length} initiatives`);
      ...
      initiatives.forEach(initiative => {
        if (!initiative || !initiative.key) {
          console.warn('⚠️ Skipping invalid initiative entry (null or missing key)');
          return;
        }
      ...
      ss.toast('Writing Initiative data...', '📝 Processing', 10);
      ...
      ss.toast(`✅ Enrichment complete! Updated ${enrichedCount} rows.`, '✅ Success', 5);
    ...
    try {
      ...
      if (perspective && typeof perspective === 'string') {
        ...
        if (cleaned.length > 10 && cleaned.length < 800 && !hasBullets) {
          console.log(`✓ Generated ${perspectiveType} perspective for ${initiativeKey}`);
          return cleaned;
        }
...
function mergeMultiplePerspectives(perspectives, epics, perspectiveType) {
  ...
  7. NEVER use bullet points (•), dashes (-), or numbered lists
  ...
  ✓ "Building OAuth 2.0 authentication with SSO integration across three platforms"
  ✓ "Migrating payment processing to microservices architecture with real-time reconciliation"
  ✓ "Implementing AI-powered denial detection engine with automated claim resubmission workflows"

  BAD EXAMPLES:
  ✗ "This PI will focus on authentication" (forbidden phrase)
  ✗ "• OAuth • SSO • Multi-factor" (bullets forbidden)
  ✗ "Feature 1: Auth, Feature 2: SSO" (enumeration forbidden)
    ...
    try {
      ...
      if (mergedPerspective && typeof mergedPerspective === 'string') {
        ...
        if (!hasError && !hasBullets && !hasMultipleSentences && wordCount <= 20 && cleaned.length > 10) {
          console.log(`✓ Merged ${perspectives.length} ${perspectiveType} perspectives (${wordCount} words)`);
          return cleaned;
        }
...
function enrichWithInitiativeDataOnSheet(targetSheet) {
  const ss = targetSheet.getParent();
  const sheet = targetSheet;
  
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log('║           INITIATIVE ENRICHMENT (On Sheet)                   ║');
  console.log('╠══════════════════════════════════════════════════════════════╣');
  console.log(`║  Sheet: ${sheet.getName()}  |  ${new Date().toLocaleString()}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  try {
    ...
    if (parentKeyCol === 0 || issueKeyCol === 0 || issueTypeCol === 0) {
      console.error('Could not find required columns (Key, Parent Key, Issue Type)');
      return;
    }
    ...
    console.log('Fetching initiative data from JIRA...');
    const initiatives = fetchInitiativesByKeys(Array.from(initiativeKeys));
    
    if (!initiatives || initiatives.length === 0) {
      console.warn('No initiatives returned from JIRA');
      return;
    }
